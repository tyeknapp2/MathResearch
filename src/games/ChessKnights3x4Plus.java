...
public class ChessKnights3x4Plus implements Game {
	...
	/**
	 * Checks and sets the victory status of the game. A game of knights is considered won if one of either two knights have been eliminated.
	...
	/**
	 * 	Returns a list of boards based on moving the piece specified in the parameter. It moves that color knight to every possible position it could be in for the next turn and outputs the string used to instantiate the next turn. 
	 *	@param turn	The piece that you wish to move. Must match <code>toMove</code>.
	 *	@throws TurnMismatchError	If <code>turn</code> does not match <code>toMove</code> then it throws this error
	 *	@returns	An <code>ArrayList</code> of <code>Strings</code> used to instantiate new <code>ChessKnights3x4Plu</code> that represent the next turns after moving the knight of the color specified by <code>turn</code>.
	 */
	@Override
	...
	/**
	 * @param indexInString index of the piece to move in the <code>board</code> <code>String</code>. 
	 * @return
	 */
