			...
			if(!game.checkStalemateStatus()&&!game.checkVictory()) {
				...
				for(String b: game.possibleMoves(player)) {
					if(game instanceof TicTacToe3x3)
						adjacencyList.get(temp).add(createAdjacencyList(new TicTacToe3x3(b), (turn==1?game.getPlayer2():game.getPlayer1()) ,(turn==1? 2:1)));
					else if(game instanceof ChessKings3x3) {
						adjacencyList.get(temp).add(createAdjacencyList(new ChessKings3x3(b), (turn==1?game.getPlayer2():game.getPlayer1()) ,(turn==1? 2:1)));
...
////		System.out.println("Keeeeeeek: "+((new ChessKings3x3()).possibleMoves(ChessKings3x3.PLAYER_1)));
////		System.out.println("Keeeeeeek: "+((new ChessKings3x3()).possibleMoves(ChessKings3x3.PLAYER_2)));
//		System.out.println("eeWeeeeBe: "+((new ChessKings3x3("eeWeeeeBe")).possibleMoves(ChessKings3x3.PLAYER_1)));
//		System.out.println("eeWeeeeBe: "+((new ChessKings3x3("eeWeeeeBe")).possibleMoves(ChessKings3x3.PLAYER_2)));
////		System.out.println("WeeeeeBee: "+((new ChessKings3x3("eWeeeeBee")).possibleMoves(ChessKings3x3.PLAYER_1)));
////		System.out.println("WeeeeeBee: "+((new ChessKings3x3("eWeeeeBee")).possibleMoves(ChessKings3x3.PLAYER_2)));
	}
	...
	public static void test3x3()
	{
		...
		while(JOptionPane.showConfirmDialog(null, testBoard.toString()+"\n\nVICTORY:"+testBoard.seeVictoryStatus()+"\nSTALEMATE:"+testBoard.seeStalemateStatus(), "Board Check", JOptionPane.YES_NO_CANCEL_OPTION)==JOptionPane.YES_OPTION) {
			String s= "";
			for(int i=0;i<9;i++) {
